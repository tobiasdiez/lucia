...
export const testAdapter = async (adapter: Adapter, database: Database) => {
	...
	await method("setUser()", async (test) => {
		...
		await test("Inserts user and key, with user id generated by db", async () => {
			const user = database.generateUser();
			const key = database.generateKey(user.id);
			const { id: _, ...userWithoutId } = user;
			const { user_id: __, ...keyWithoutUserId } = key;
			const userReturned = await adapter.setUser(
				userWithoutId,
				keyWithoutUserId
			);
			const storedUser = await User.get(userReturned.id);
			const storedKey = await Key.get(key.id);
			assert.deepStrictEqual(storedUser, userReturned);
			assert.deepStrictEqual(
				{ ...userWithoutId, id: userReturned.id },
				userReturned
			);
			assert.deepStrictEqual(
				{
